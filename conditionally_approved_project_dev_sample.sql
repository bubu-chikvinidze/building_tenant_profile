...
with approval_statuses_explicit as (
    ...
    where 1 = 1
        ...
        and b.application_create_date  < '2022-04-15' -- fixing final date of apps for dev sample
...
, duplicated_rules as (
    select 
         z.id
        ,count(1) cnt
    from "REPORTS".DBT_BCHIKVINIDZE.BUILDING_RULES_2 z 
...
where 1=1 
    ...
    and d.application_create_date < '2022-04-15'
...
, explicit_vs_inferred as (
    ...
    left join "REPORTS".DBT_BCHIKVINIDZE.BUILDING_RULES_2 z on z.id = a.buildingid
...
/*, explicit_and_inferred_statuses as (
     select
        ...
        ,'explicit' status_logic_source_type
        ,null inference_type
        ,if_inferred_APPROVAL_STATUS
        ,if_inferred_inference_type
    from explicit_vs_inferred a
    where a.id not in (select id from approval_statuses_inferred) --don't want to dublicate inferred approval status
    ...
    select 
        ...
        ,inference_type
        ,null if_inferred_APPROVAL_STATUS
        ,null if_inferred_inference_type
...
, building_profile as (
    select --3.3k buildings with at least 1 apps, 
         a.buildingid
        ,a.units
        ,round(avg(a.monthly_rent), 2) avg_monthly_rent
        ,a.latitude
        ,a.longitude
        ,a.zip
        ,a.landlord_source_id
        ...
        ,sum(case when approval_status = 'Approved with conditions' then 1 else 0 end) conditional_apps_cnt
        ...
        ,sum(case when status_logic_type = 'available' and approval_status = 'Approved with conditions' then 1 else 0 end) cnt_conditional_explicit_apps
        ,sum(case when (INFERENCE_TYPE = 'clear' or status_logic_type = 'available') and approval_status = 'Approved with conditions' then 1 else 0 end) cnt_conditional_clear_and_explicit_apps
    
        ,case when cnt_status_available_apps <> 0 then
            round(conditional_apps_cnt/cnt_status_available_apps, 2) else null end conditional_over_available
        ,case when cnt_explicit_apps <> 0 then
            round(cnt_conditional_explicit_apps/cnt_explicit_apps, 2) else null end conditional_over_available_explicit
        ,case when cnt_clear_and_explicit_apps <> 0 then
            round(cnt_conditional_clear_and_explicit_apps/cnt_clear_and_explicit_apps, 2) else null end conditional_over_available_explicit_and_clear
...
/* Matching of DW building to ALN Management company. 
   ...
   Note: The management companies that were matched by our logic but in reality don't have corresponding building in ALN
   at all will be filtered out even if the landlords were matched. The filtering happend when we join buildings and ALN LLs by corporate ID. 
*/
, landlord_dw_to_aln_match_by_datateam as (
    ...
    where 1=1
    and a.id_tg not in (select id_tg from dup_landlord_dw_to_aln_match_by_datateam)
    and e.source_id not in (select source_id from building_to_aln_company_match3)
    and e.source_id not in (select source_id from building_to_aln_company_match2)
    and e.source_id not in (select source_id from building_to_aln_company_match1)
    group by e.source_id, aln_id, landlord_source_id
)
/* Adding landlord matching from already matched buildings*/
, landlord_matching_from_building_matching as (
    select 
         a.id_mon source_id
        ,null hubspot_buildingId --adding these columns so that it aligns with other UNIONs
        ,null hubspot_parentId
        ,max(c."MgmtOfficeIntegerId") aln_id -- for removing dupes if any exist (4 now, possible data entry error)
        ,max(e.landlord_source_id) landlord_source_id
    from dw_to_aln_building_matching a 
    JOIN "EXT_DATA"."ALN_DATA"."ApartmentPropertyExtension" b on a.id_aln = b."ApartmentId"
    JOIN "EXT_DATA"."ALN_DATA"."ManagementCompany" c on c."ManagementCompanyEntityId" = b."CorporateManagementCompanyId"
    join TG_DW_DB.public.building e on e.source_id = a.id_mon
    where 1=1
        and e.source_id not in (select source_id from landlord_dw_to_aln_match_by_datateam)
        and e.source_id not in (select source_id from building_to_aln_company_match3)
        and e.source_id not in (select source_id from building_to_aln_company_match2)
        and e.source_id not in (select source_id from building_to_aln_company_match1)
    group by a.id_mon
...
, building_to_aln_company_match as (
    ...
    --    union - this is commented out because did not improve model performance - on the contrary. 
    --select * from landlord_matching_from_building_matching e
    /*where 1=1
...
, pms as (
    SELECT 
         a."ApartmentId" ApartmentId
        ,case when "Property_PricingTier" = 'A' then 1 
              when "Property_PricingTier" = 'B' then 2
              when "Property_PricingTier" = 'C' then 3
              when "Property_PricingTier" = 'D' then 4 end Property_PricingTier
        ,case when "Property_AptHomePage" is not null then 1 else 0 end homepage_available
        ,case when lower("PMSoftwareNotes") is not null then 1 else 0 end PMS_AVAILABLE
        ,case when lower("PMSoftwareNotes") like '%yardi%' then 1 else 0 end PMS_YARDI
        ,case when lower("PMSoftwareNotes") like '%crm%' then 1 else 0 end PMS_CRM
        ,case when lower("PMSoftwareNotes") like '%rent cafe%' then 1 else 0 end PMS_RENT_CAFE
        ,case when lower("PMSoftwareNotes") like '%onesite%' or lower("PMSoftwareNotes") like '%onsite%' then 1 else 0 end PMS_ONESITE
        ,case when lower("PMSoftwareNotes") like '%fortress%' then 1 else 0 end PMS_FORTRESS
        ,case when "Property Type" = 'Garden' then 1 else 0 end property_type_garden
        ,case when "Property Type" = 'Mid-Rise' then 1 else 0 end property_type_midrise
        ,case when "Property Type" = 'Hi-Rise' then 1 else 0 end property_type_hirise
    FROM EXT_DATA.ALN_BACKEND.FTP_PROPERTIES a
)
...
,a.conditional_over_available target
,a.denied_over_available denied_percentage
--,r."MgmtOfficeIntegerId" MgmtOfficeIntegerId
--,a.landlord_source_id
,a.conditional_over_available_explicit target_explicit
,a.conditional_over_available_explicit_and_clear target_explicit_and_clear
...
,case when v.PMS_AVAILABLE is null then 0 else v.PMS_AVAILABLE end PMS_AVAILABLE
,case when v.PMS_YARDI is null then 0 else v.PMS_YARDI end PMS_YARDI
,case when v.PMS_CRM is null then 0 else v.PMS_CRM end PMS_CRM
,case when v.PMS_RENT_CAFE is null then 0 else v.PMS_RENT_CAFE end PMS_RENT_CAFE
,case when v.PMS_ONESITE is null then 0 else v.PMS_ONESITE end PMS_ONESITE
,case when v.PMS_FORTRESS is null then 0 else v.PMS_FORTRESS end PMS_FORTRESS
,v.Property_PricingTier
,case when v.homepage_available is null then 0 else v.homepage_available end homepage_available
,case when v.property_type_garden is null then 0 else v.property_type_garden end property_type_garden
,case when v.property_type_midrise is null then 0 else v.property_type_midrise end property_type_midrise
,case when v.property_type_hirise is null then 0 else v.property_type_hirise end property_type_hirise
from building_profile a
...
left join pms v on v.apartmentid = s.apartmentid
where 1=1;
